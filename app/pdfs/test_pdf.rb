class TestPdf < Prawn::Document
  ...
  def initialize(client, settings)
    ...
    self.draw_text("Garnishee",  at:[58,385], style: :bold)

    #NOTICE TO INDIVIDUAL DEFENDANT FOR CLAIM OF EXEMPTION FROM GARNISHMENT PURSUANT TO FLORIDA STATUTE 77.041
    self.draw_text("#{@client.owes_money_first_name} #{@client.owes_money_last_name}",  at:[110,260], style: :bold)
    self.draw_text(@client.mailing_address,  at:[123,246], style: :bold)
    ...
    self.draw_text("#{@client.owes_money_first_name} #{@client.owes_money_last_name}", at:[88,450], style: :bold)
